...
public class RoupaVendaDao extends Dao implements DaoI<RoupaVenda> {
    ...
    public List<RoupaVenda> listar() {
        try {
            ...
            stmt = conexao.prepareStatement("select IDLOCACAO, IDROUPA,TIPODEVENDA from roupavenda"
            ...
            while (result.next()) {
                ...
                rv.setidLocacao(result.getInt("IDLOCACAO"));
                rv.setTipoDeVenda(result.getInt("TIPODEVENDA"));
                rv.setIdRoupa(result.getInt("IDROUPA"));
                lista.add(rv);
            }
            return lista;
    ...
    public int cadastrar(RoupaVenda obj) {
        try {
            ...
            stmt = conexao.prepareStatement(
                    "insert into roupavenda(IDROUPA,TIPODEVENDA,IDLOCACAO)"
                    + " values(?,?,?)", PreparedStatement.RETURN_GENERATED_KEYS);
            stmt.setInt(1, obj.getIdRoupa());
            stmt.setInt(2, obj.getTipoDeVenda());
            stmt.setInt(3, obj.getidLocacao());
            ResultSet rs;
